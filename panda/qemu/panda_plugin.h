...
typedef enum panda_cb_type {
    ...
    // GA Added for Faros
    PANDA_CB_FAROS_NET_RECV,
    PANDA_CB_FAROS_NET_SEND,
    PANDA_CB_LAST
} panda_cb_type;
 ...
 */
   ...
   * GA FAROS
   *
   * Callback ID: PANDA_CB_FAROS_NET_RECV,
   *
   *    Added for FAROS project to allow handling of received network packets
   *    (only supports e1000)
   *
   * Arguments:
   *    CPUState *env:      pointer to CPUState
   *    uint64_t dst_addr:  address where received data is dma'd
   *    uint32_t num_bytes: size of data in bytes
   *
   * Return value:
   *    unused
   */
   ...
   * GA FAROS
   *
   * Callback ID: PANDA_CB_FAROS_NET_SEND,
   *
   *    Added for FAROS project to allow handling of sent network packets
   *    (only supports e1000)
   *
   * Arguments:
   *    CPUState *env:      pointer to CPUState
   *    uint64_t src_addr:  address where received data is dma'd
   *    uint32_t num_bytes: size of data in bytes
   *
   * Return value:
   *    unused
